...
class FeatureEngineer:
    ...
    def create_training_dataset(
        ...
        """
        logger.info(f"🔧 특성 생성 시작... (총 {len(matching_results)}건)")
        ...
        logger.info(f"✅ 특성 생성 완료!")
        logger.info(f"   - 특성 수: {len(self.feature_columns)}개")
        logger.info(f"   - 샘플 수: {len(X)}개")
    ...
    def create_features_from_db_data(
        ...
        """
        DB 데이터에서 직접 특성 생성 (CSV 없이 사용)

        Args:
            patient_personality: 환자 성격 점수 딕셔너리
            caregiver_data: 간병인 정보 딕셔너리
        ...
        # 2. 전문분야 일치율 (DB에서는 간략화)
        features["specialty_match_ratio"] = 0.5  # 기본값

        # 3. 지역 일치 점수 (DB에서는 간략화)
        features["region_match_score"] = 0.5  # 기본값
